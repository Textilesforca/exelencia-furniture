export const translations = {
  es: {
    navbar: {
      inicio: 'Inicio',
      catalogo: 'Catálogo',
      cotizar: 'Cotizar',
      buscarPlaceholder: 'Busca aquí por todo el catálogo…',
      carritoAria: 'Carrito',
      delivery: 'Entrega y recolección',
      contactUs: 'Contáctanos',
      admin: 'Admin',
      panelAdmin: 'Panel de administración',
      iniciarSesion: 'Iniciar sesión',
      buscando: 'Buscando…',
      abrirMenu: 'Abrir menú',
      cerrarMenu: 'Cerrar menú',
    },
    cart: {
      titulo: 'Tu carrito',
      vacio: 'Tu carrito está vacío.',
      total: 'Total',
      pagar: 'Pagar',
      procesando: 'Procesando…',
    },
    footer: {
      taller: 'Taller y showroom con cita previa',
      derechos: 'Todos los derechos reservados.',
    },
    hero: {
      kicker: 'Diseño · Medida · Materia',
      tituloL1: 'Ayudamos a nuestros clientes a hacer realidad sus sueños mediante la fabricación de muebles',
      tituloL2: 'personalizados o de catálogo.',
      texto:
        'Cada lugar de tu hogar merece ser excepcional. Tenemos una amplia selección de muebles de alta calidad y también ofrecemos muebles personalizados hechos por nuestras manufacturas para hacer de cada espacio un lugar inolvidable.',
      verCatalogo: 'Ver catálogo',
      pedirCotizacion: 'Pedir cotización',
      bocetoAlt: 'Boceto acotado de un sillón a medida',
      refSillonL1: 'Descubre las dimensiones de',
      refSillonL2: 'Salas, Comedores, Recámaras',
    },
    home: {
      destacadas: 'Lo más vendido',
      paso1Titulo: 'Estimados y Presupuestos',
      paso1Texto:
        'Nuestros expertos pueden ir a tu hogar en el área de Los Ángeles, Gardena, San Pedro, etc. para ayudarte a medir y diseñar tu espacio de manera óptima, o nos puedes compartir tus medidas y te daremos un presupuesto.',
      paso2Titulo: 'Materiales y Productos',
      paso2Texto:
        'Contamos con material de excelencia para diseñar tus muebles personalizados, junto con productos de catálogo para renovar tus espacios.',
      paso3Titulo: 'Entregas o Pickup',
      paso3Texto:
        'Contamos con servicios profesionales de entrega e instalación en California, o pickup directamente de nuestra localidad.',
      ctaTitulo: '¿Tienes un proyecto innovador que deseas hacer realidad?',
      ctaBoton: 'Cuéntanos tu proyecto',
    },
    catalog: {
      cargando: 'Cargando catálogo…',
      vacio: 'Aún no hay piezas en esta categoría.',
      sinFotos: 'Aún no hay fotos de catálogo en esta categoría.',
      filtroAria: 'Filtrar por categoría',
      verCatalogo: 'Ver catálogo',
      piezasMedidaKicker: 'Piezas hechas para tu espacio',
      piezasMedidaIntro:
        'Cada lugar de tu hogar merece ser excepcional. Tenemos una amplia selección de muebles de alta calidad y también ofrecemos muebles personalizados hechos por nuestras manufacturas para hacer de cada espacio un lugar inolvidable.',
    },
    categorias: {
      Todos: 'Todos',
      Salas: 'Salas',
      Comedores: 'Comedores',
      Recámaras: 'Recámaras',
      'Piezas a medida': 'Piezas a medida',
    },
    subcategorias: {
      Salas: {
        'Todas las Salas': 'Todas las Salas',
        'Sala Modular': 'Sala Modular',
        'Juego de Salas': 'Juego de Salas',
        'Sala con reclinable': 'Sala con reclinable',
        'Sofá 3 plazas': 'Sofá 3 plazas',
        'Sofá 2 plazas': 'Sofá 2 plazas',
        'Sala de Tela': 'Sala de Tela',
        'Sala de Piel y Ecopiel': 'Sala de Piel y Ecopiel',
        'Sofá Camas': 'Sofá Camas',
        Sillones: 'Sillones',
        Taburetes: 'Taburetes',
        'Sillones Reclinables': 'Sillones Reclinables',
      },
      Comedores: {
        'Todos los comedores': 'Todos los comedores',
        'Mesa + 4 sillas': 'Mesa + 4 sillas',
        'Mesa + 6 sillas': 'Mesa + 6 sillas',
        'Mesa de Comedor': 'Mesa de Comedor',
        Bar: 'Bar',
        Sillas: 'Sillas',
        Bancas: 'Bancas',
        Bancos: 'Bancos',
        'Aparador & Vitrina': 'Aparador & Vitrina',
        'Comedores de jardín': 'Comedores de jardín',
      },
      Recámaras: {
        'Todas las recámaras': 'Todas las recámaras',
        'Recámara 3 piezas': 'Recámara 3 piezas',
        'Recámara 4 piezas': 'Recámara 4 piezas',
        'Recámara 5 piezas': 'Recámara 5 piezas',
        Camas: 'Camas',
        Cabeceras: 'Cabeceras',
        'Bases para Cama': 'Bases para Cama',
        Burós: 'Burós',
        Cajoneras: 'Cajoneras',
        'Armario o Clóset': 'Armario o Clóset',
        'Baúl o Pie de cama': 'Baúl o Pie de cama',
        'Muebles de T.V': 'Muebles de T.V',
        'Espejos para Recámara': 'Espejos para Recámara',
        'Set cajonera y espejo': 'Set cajonera y espejo',
        Iluminación: 'Iluminación',
      },
    },
    productCard: {
      fichaTecnica: 'Ficha técnica',
      ancho: 'Ancho',
      alto: 'Alto',
      fondo: 'Fondo',
      desde: 'Desde',
      disponibleEn: 'Disponible en:',
    },
    productDetail: {
      cargando: 'Cargando pieza…',
      noEncontrada: 'No encontramos esta pieza.',
      volverCatalogo: 'Volver al catálogo',
      fichaTecnica: 'Ficha técnica',
      ancho: 'Ancho',
      alto: 'Alto',
      fondo: 'Fondo',
      desde: 'Desde',
      comprar: 'Comprar ahora',
      redirigiendo: 'Redirigiendo…',
      cotizarMedida: 'Cotizar a mi medida',
      color: 'Color',
      cantidad: 'Cantidad',
      agregarCarrito: 'Agregar al carrito',
      agregado: 'Agregado ✓',
      sinExistencias: 'Sin existencias por el momento',
      metodoEnvio: 'Método de envío',
      recogerTienda: 'Recoger en tienda',
      servicioFlete: 'Servicio de flete',
      totalConEnvio: 'Total con envío',
    },
    lightbox: { cerrar: 'Cerrar ✕' },
    contact: {
      titulo: 'Contáctanos',
      intro:
        'Nuestros expertos pueden ir a tu hogar en el área de Los Ángeles, Gardena, San Pedro, etc. para ayudarte a medir y diseñar tu espacio de manera óptima, o nos puedes compartir tus medidas y te realizamos tu presupuesto.',
      materialesTitulo: 'Materiales y Productos',
      materialesTexto:
        'Contamos con materiales de excelencia para diseñar tus muebles personalizados, junto con productos de catálogo para renovar tus espacios.',
      telefono: 'Teléfono:',
      whatsapp: 'WhatsApp:',
      horario: 'Horario:',
      horarioValor: 'Lun - Sáb: 9:00am - 6:30pm　　Dom: 10:30am - 5:00pm',
      formTitulo: 'Cuéntanos qué mueble necesitas',
      formTexto:
        'Cuanto más nos digas sobre el espacio, el uso diario y tus referencias, más preciso será el primer plano que te mostremos. Respondemos en menos de 24 horas.',
    },
    delivery: {
      titulo: 'Entregas o Pickup',
      intro:
        'Contamos con servicios profesionales de entrega e instalación en California, o pickup directamente de nuestra localidad.',
      servicioProfesional: 'Servicios profesionales',
      pickup: 'Pickup',
      telefono: 'Teléfono:',
      whatsapp: 'WhatsApp:',
    },
    quoteForm: {
      exitoTitulo: 'Solicitud recibida',
      exitoTexto: 'Gracias, {nombre}. Te contactaremos en menos de 24 horas para afinar medidas y materiales.',
      nombre: 'Nombre',
      telefono: 'Teléfono / WhatsApp',
      correo: 'Correo (opcional)',
      tipoMueble: 'Tipo de mueble',
      tipoMueblePlaceholder: 'Ej. mesa de comedor, clóset, sala',
      descripcion: 'Cuéntanos qué necesitas',
      descripcionPlaceholder:
        'Medidas del espacio, estilo, material preferido, fotos de referencia (puedes enviarlas luego por WhatsApp)',
      presupuesto: 'Presupuesto aproximado',
      presupuestoPlaceholder: 'Ej. $1,500 - $2,500 USD',
      errorEnvio: 'No se pudo enviar tu solicitud. Verifica tu conexión e inténtalo de nuevo.',
      enviando: 'Enviando…',
      solicitar: 'Solicitar cotización',
    },
    paymentSuccess: {
      kicker: 'Pago recibido',
      titulo: '¡Gracias por tu pago!',
      confirmando: 'Confirmando…',
      estado: 'Estado',
      noEncontrado: 'No pudimos encontrar el detalle de este pago, pero si tu banco confirmó el cargo, ya lo recibimos.',
      volverCatalogo: 'Volver al catálogo',
      anticipo: 'Anticipo',
      resto: 'Resto del pago',
      descargarRemision: 'Descargar remisión',
      remisionTitulo: 'Remisión',
      remisionAviso: 'Este documento es un comprobante de pago y no tiene validez como factura fiscal.',
      folio: 'Folio',
      fecha: 'Fecha',
      cliente: 'Cliente',
      clienteGenerico: 'Cliente',
      descripcion: 'Descripción',
      cantidad: 'Cantidad',
      precioUnitario: 'Precio unitario',
      importe: 'Importe',
      total: 'Total',
      gracias: '¡Gracias por tu compra!',
      servicioFlete: 'Servicio de flete',
      notaFleteTitulo: 'Servicio completo de entrega',
      notaFleteItems: [
        'Colocación de la(s) pieza(s) en la habitación que elijas',
        'Ensamblaje de todos los artículos',
        'Retiro de cajas y escombros',
        'Entrega el mismo día disponible en ciertas zonas para compras realizadas antes de las 12:00 pm',
        'No incluye retiro o traslado de muebles existentes',
      ],
    },
    paymentCancel: {
      kicker: 'Pago no completado',
      titulo: 'Cancelaste el pago',
      texto: 'No se realizó ningún cargo. Puedes intentarlo de nuevo cuando quieras.',
      volverCatalogo: 'Volver al catálogo',
    },
    admin: {
      cargando: 'Cargando…',
      cerrarSesion: 'Cerrar sesión',
      panelPrivado: 'Panel privado',
      administracion: 'Administración',
      sinPermisos: 'No tienes permisos asignados. Contacta al administrador.',
      tabProductos: 'Productos',
      tabCotizaciones: 'Cotizaciones',
      tabBanner: 'Banner',
      tabCatalogo: 'Catálogo',
      tabInventario: 'Inventario',
      tabVentas: 'Ventas',
      tabEnvio: 'Envío',
      tabUsuarios: 'Usuarios',
    },
    loginForm: {
      titulo: 'Acceso restringido',
      subtitulo: 'Panel de administración',
      correo: 'Correo',
      contrasena: 'Contraseña',
      errorCredenciales: 'Correo o contraseña incorrectos.',
      entrando: 'Entrando…',
      entrar: 'Entrar',
    },
    productManager: {
      confirmarEliminar: '¿Eliminar esta pieza del catálogo?',
      editarPieza: 'Editar pieza',
      nuevaPieza: 'Nueva pieza',
      nombre: 'Nombre',
      nombreEn: 'Nombre (inglés, opcional)',
      categoria: 'Categoría',
      subcategoria: 'Subcategoría',
      sinSubcategoria: 'Sin subcategoría',
      material: 'Material',
      materialEn: 'Material (inglés, opcional)',
      anchoCm: 'Ancho (in)',
      altoCm: 'Alto (in)',
      fondoCm: 'Fondo (in)',
      precioDesde: 'Precio desde (USD)',
      descripcion: 'Descripción',
      descripcionEn: 'Descripción (inglés, opcional)',
      imagen: 'Imagen',
      imagenActualAlt: 'Actual',
      galeria: 'Galería de fotos (adicionales)',
      colores: 'Colores disponibles',
      agregarColor: 'Agregar color',
      colorNombrePlaceholder: 'Ej. Beige',
      colorNombreEnPlaceholder: 'Nombre en inglés (opcional)',
      guardadoExito: 'Guardado correctamente ✓',
      guardando: 'Guardando…',
      guardarCambios: 'Guardar cambios',
      agregarPieza: 'Agregar pieza',
      cancelar: 'Cancelar',
      catalogoActual: 'Catálogo actual',
      cargando: 'Cargando…',
      vacio: 'Aún no hay piezas.',
      todasSubcategorias: 'Todas las subcategorías',
      sinResultadosFiltro: 'No hay piezas con este filtro.',
      editar: 'Editar',
      eliminar: 'Eliminar',
    },
    quotesList: {
      cargando: 'Cargando…',
      vacio: 'Aún no hay solicitudes de cotización.',
      confirmarEliminar: '¿Eliminar esta cotización?',
      montoInvalido: 'Ingresa un monto válido.',
      eliminar: 'Eliminar',
      telefono: 'Teléfono:',
      correo: 'Correo:',
      tipoMueble: 'Tipo de mueble:',
      presupuesto: 'Presupuesto:',
      montoTotalPlaceholder: 'Precio total de la pieza (USD, opcional)',
      anticipoRecibido: 'Anticipo recibido ✓ —',
      montoPlaceholder: 'Monto del anticipo (USD)',
      restoRecibido: 'Resto recibido ✓ —',
      montoRestoPlaceholder: 'Monto del resto (USD)',
      generarLinkResto: 'Generar link de pago del resto',
      generando: 'Generando…',
      generarLink: 'Generar link de pago',
      copiar: 'Copiar',
    },
    userManager: {
      nuevoUsuario: 'Nuevo usuario',
      correo: 'Correo',
      contrasena: 'Contraseña',
      rol: 'Rol',
      usuarioEstandar: 'Usuario estándar',
      adminRol: 'Admin',
      accesoSecciones: 'Acceso a secciones',
      productos: 'Productos',
      cotizaciones: 'Cotizaciones',
      banner: 'Banner',
      catalogo: 'Catálogo',
      inventario: 'Inventario',
      ventas: 'Ventas',
      envio: 'Envío',
      creando: 'Creando…',
      crearUsuario: 'Crear usuario',
      usuariosExistentes: 'Usuarios existentes',
      cargando: 'Cargando…',
      vacio: 'Aún no hay usuarios.',
      guardando: 'Guardando…',
      guardar: 'Guardar',
      eliminar: 'Eliminar',
      confirmarEliminar: '¿Eliminar este usuario? Perderá acceso de inmediato.',
      mostrarPassword: 'Mostrar contraseña',
      ocultarPassword: 'Ocultar contraseña',
    },
    inventoryManager: {
      titulo: 'Inventario',
      cargando: 'Cargando…',
      vacio: 'Aún no hay piezas en el catálogo.',
      sinExistencias: 'Sin existencias',
      bajoStock: 'Stock bajo',
      guardando: 'Guardando…',
      guardado: 'Guardado ✓',
      guardar: 'Guardar',
      guardarTodo: 'Guardar todo',
      errorGuardarTodo: 'Algunos cambios no se pudieron guardar.',
    },
    salesManager: {
      titulo: 'Ventas',
      cargando: 'Cargando…',
      vacio: 'No hay ventas en este período.',
      hoy: 'Hoy',
      semana: 'Semanal',
      mes: 'Mes',
      anio: 'Año',
      fecha: 'Fecha',
      tipo: 'Tipo',
      descripcion: 'Descripción',
      cliente: 'Cliente',
      monto: 'Monto',
      total: 'Total',
      tipoProducto: 'Compra directa',
      tipoCarrito: 'Carrito',
      tipoAnticipo: 'Anticipo',
      generarReporte: 'Generar reportes PDF',
      generando: 'Generando…',
      reporteTitulo: 'Reporte de ventas',
      reportePiezasTitulo: 'Reporte de piezas vendidas',
      periodo: 'Período',
      generadoEl: 'Generado el',
      vistaDinero: 'Dinero',
      vistaPiezas: 'Piezas vendidas',
      categoria: 'Categoría',
      piezasVendidas: 'Piezas vendidas',
      expandir: 'Ver subcategorías',
      colapsar: 'Ocultar subcategorías',
    },
    shippingManager: {
      titulo: 'Envío',
      descripcion: 'Configura el monto del servicio de flete que se muestra antes de pagar.',
      cargando: 'Cargando…',
      montoFlete: 'Monto del flete (USD)',
      guardando: 'Guardando…',
      guardar: 'Guardar',
      guardadoExito: 'Guardado correctamente ✓',
    },
    bannerManager: {
      cargando: 'Cargando…',
      tituloInicio: 'Banner de Inicio (carrusel)',
      descripcionInicio: 'Estas imágenes rotan en el banner de la página de Inicio. Agrega o quita las que quieras.',
      tituloCatalogo: 'Banner del Catálogo (imagen fija)',
      descripcionCatalogo: 'Una sola imagen fija que se muestra arriba del listado en /catalogo.',
      agregarImagenes: 'Agregar imágenes',
      cambiarImagen: 'Cambiar imagen',
      subiendo: 'Subiendo…',
      confirmarQuitar: '¿Quitar esta imagen del banner?',
      errorSubir: 'No se pudo subir la imagen',
    },
    catalogoManager: {
      cargando: 'Cargando…',
      descripcion:
        'La portada es la foto de la tarjeta al entrar a /catalogo. La galería son varias fotos que se muestran al entrar a esa categoría.',
      portada: 'Portada (1 foto)',
      sinPortada: 'Sin portada todavía',
      subirPortada: 'Subir portada',
      galeria: 'Galería del catálogo (varias fotos)',
      sinGaleria: 'Sin fotos todavía',
      agregarGaleria: 'Agregar fotos',
      subiendo: 'Subiendo…',
      confirmarQuitar: '¿Quitar esta imagen?',
      errorSubir: 'No se pudo subir la imagen',
    },
    idioma: { cambiarA: 'EN' },
  },

  en: {
    navbar: {
      inicio: 'Home',
      catalogo: 'Catalog',
      cotizar: 'Get a Quote',
      buscarPlaceholder: 'Search the whole catalog…',
      carritoAria: 'Cart',
      delivery: 'Delivery and Pickup',
      contactUs: 'Contact Us',
      admin: 'Admin',
      panelAdmin: 'Admin panel',
      iniciarSesion: 'Log in',
      buscando: 'Searching…',
      abrirMenu: 'Open menu',
      cerrarMenu: 'Close menu',
    },
    cart: {
      titulo: 'Your cart',
      vacio: 'Your cart is empty.',
      total: 'Total',
      pagar: 'Pay',
      procesando: 'Processing…',
    },
    footer: {
      taller: 'Workshop and showroom by appointment',
      derechos: 'All rights reserved.',
    },
    hero: {
      kicker: 'Design · Measure · Material',
      tituloL1: 'We help our clients bring their dreams to life by crafting',
      tituloL2: 'custom or catalog furniture.',
      texto:
        'Every place in your home deserves to be exceptional. We offer a wide selection of high-quality furniture and also custom pieces built by our own workshop to make every space unforgettable.',
      verCatalogo: 'View catalog',
      pedirCotizacion: 'Request a quote',
      bocetoAlt: 'Dimensioned sketch of a custom armchair',
      refSillonL1: 'Discover the dimensions of',
      refSillonL2: 'Living, Dining, Bedroom sets',
    },
    home: {
      destacadas: 'Best sellers',
      paso1Titulo: 'Estimates and Budgets',
      paso1Texto:
        'Our experts can visit your home in the Los Angeles, Gardena, San Pedro area, etc. to help you measure and design your space optimally, or you can share your own measurements and we will give you a quote.',
      paso2Titulo: 'Materials and Products',
      paso2Texto:
        'We use top-quality materials to design your custom furniture, along with catalog products to renew your spaces.',
      paso3Titulo: 'Deliveries or Pickup',
      paso3Texto:
        'We offer professional delivery and installation services across California, or pickup directly from our location.',
      ctaTitulo: 'Have an innovative project you want to bring to life?',
      ctaBoton: 'Tell us about your project',
    },
    catalog: {
      cargando: 'Loading catalog…',
      vacio: 'No pieces in this category yet.',
      sinFotos: 'No catalog photos in this category yet.',
      filtroAria: 'Filter by category',
      verCatalogo: 'View catalog',
      piezasMedidaKicker: 'Pieces made for your space',
      piezasMedidaIntro:
        'Every place in your home deserves to be exceptional. We offer a wide selection of high-quality furniture and also custom pieces built by our own workshop to make every space unforgettable.',
    },
    categorias: {
      Todos: 'All',
      Salas: 'Living Rooms',
      Comedores: 'Dining Rooms',
      Recámaras: 'Bedrooms',
      'Piezas a medida': 'Custom Pieces',
    },
    subcategorias: {
      Salas: {
        'Todas las Salas': 'All Living Rooms',
        'Sala Modular': 'Modular Living Room',
        'Juego de Salas': 'Living Room Set',
        'Sala con reclinable': 'Reclining Living Room',
        'Sofá 3 plazas': '3-Seat Sofa',
        'Sofá 2 plazas': '2-Seat Sofa',
        'Sala de Tela': 'Fabric Living Room',
        'Sala de Piel y Ecopiel': 'Leather & Faux Leather Living Room',
        'Sofá Camas': 'Sofa Beds',
        Sillones: 'Armchairs',
        Taburetes: 'Stools',
        'Sillones Reclinables': 'Recliners',
      },
      Comedores: {
        'Todos los comedores': 'All Dining Rooms',
        'Mesa + 4 sillas': 'Table + 4 Chairs',
        'Mesa + 6 sillas': 'Table + 6 Chairs',
        'Mesa de Comedor': 'Dining Table',
        Bar: 'Bar',
        Sillas: 'Chairs',
        Bancas: 'Benches',
        Bancos: 'Stools',
        'Aparador & Vitrina': 'Sideboard & Display Cabinet',
        'Comedores de jardín': 'Outdoor Dining Sets',
      },
      Recámaras: {
        'Todas las recámaras': 'All Bedrooms',
        'Recámara 3 piezas': '3-Piece Bedroom Set',
        'Recámara 4 piezas': '4-Piece Bedroom Set',
        'Recámara 5 piezas': '5-Piece Bedroom Set',
        Camas: 'Beds',
        Cabeceras: 'Headboards',
        'Bases para Cama': 'Bed Frames',
        Burós: 'Nightstands',
        Cajoneras: 'Dressers',
        'Armario o Clóset': 'Wardrobe or Closet',
        'Baúl o Pie de cama': 'Trunk or Bed Bench',
        'Muebles de T.V': 'TV Furniture',
        'Espejos para Recámara': 'Bedroom Mirrors',
        'Set cajonera y espejo': 'Dresser & Mirror Set',
        Iluminación: 'Lighting',
      },
    },
    productCard: {
      fichaTecnica: 'Specs',
      ancho: 'Width',
      alto: 'Height',
      fondo: 'Depth',
      desde: 'From',
      disponibleEn: 'Available in:',
    },
    productDetail: {
      cargando: 'Loading piece…',
      noEncontrada: "We couldn't find this piece.",
      volverCatalogo: 'Back to catalog',
      fichaTecnica: 'Specs',
      ancho: 'Width',
      alto: 'Height',
      fondo: 'Depth',
      desde: 'From',
      comprar: 'Buy now',
      redirigiendo: 'Redirecting…',
      cotizarMedida: 'Quote a custom size',
      color: 'Color',
      cantidad: 'Quantity',
      agregarCarrito: 'Add to cart',
      agregado: 'Added ✓',
      sinExistencias: 'Out of stock for now',
      metodoEnvio: 'Shipping method',
      recogerTienda: 'Pick up in store',
      servicioFlete: 'Freight delivery service',
      totalConEnvio: 'Total with shipping',
    },
    lightbox: { cerrar: 'Close ✕' },
    contact: {
      titulo: 'Contact Us',
      intro:
        'Visit us at 14709 S Western Ave, Gardena, CA 90249. Thank you for your inquiry, we will get back to you as soon as possible. If you have a question about a product, please include the item number. Thanks.',
      telefono: 'Phone:',
      whatsapp: 'WhatsApp:',
      horario: 'Store Hours:',
      horarioValor: 'Mon - Sat: 9:00am - 6:30pm　　Sun: 10:30am - 5:00pm',
      formTitulo: 'Tell us what furniture you need',
      formTexto:
        'The more you tell us about the space, daily use, and references, the more accurate the first drawing we show you will be. We respond within 24 hours.',
    },
    delivery: {
      titulo: 'Deliveries or Pickup',
      intro:
        'We offer professional delivery and installation services across California, or pickup directly from our location.',
      servicioProfesional: 'Professional services',
      pickup: 'Pickup',
      telefono: 'Phone:',
      whatsapp: 'WhatsApp:',
    },
    quoteForm: {
      exitoTitulo: 'Request received',
      exitoTexto: "Thanks, {nombre}. We'll contact you within 24 hours to refine measurements and materials.",
      nombre: 'Name',
      telefono: 'Phone / WhatsApp',
      correo: 'Email (optional)',
      tipoMueble: 'Type of furniture',
      tipoMueblePlaceholder: 'E.g. dining table, closet, living room set',
      descripcion: 'Tell us what you need',
      descripcionPlaceholder:
        'Space measurements, style, preferred material, reference photos (you can send them later via WhatsApp)',
      presupuesto: 'Approximate budget',
      presupuestoPlaceholder: 'E.g. $1,500 - $2,500 USD',
      errorEnvio: 'We could not send your request. Check your connection and try again.',
      enviando: 'Sending…',
      solicitar: 'Request a quote',
    },
    paymentSuccess: {
      kicker: 'Payment received',
      titulo: 'Thank you for your payment!',
      confirmando: 'Confirming…',
      estado: 'Status',
      noEncontrado: "We couldn't find the details of this payment, but if your bank confirmed the charge, we already received it.",
      volverCatalogo: 'Back to catalog',
      anticipo: 'Deposit',
      resto: 'Remaining balance',
      descargarRemision: 'Download receipt',
      remisionTitulo: 'Receipt',
      remisionAviso: 'This document is proof of payment and is not a tax invoice.',
      folio: 'Reference',
      fecha: 'Date',
      cliente: 'Customer',
      clienteGenerico: 'Customer',
      descripcion: 'Description',
      cantidad: 'Qty',
      precioUnitario: 'Unit price',
      importe: 'Amount',
      total: 'Total',
      gracias: 'Thank you for your purchase!',
      servicioFlete: 'Freight delivery service',
      notaFleteTitulo: 'Full delivery service',
      notaFleteItems: [
        'Placement of the item(s) in the room of your choice',
        'Assembly of all items',
        'Removal of boxes and debris',
        'Same-day delivery available in certain areas for purchases made before 12:00 pm',
        'Does not include removal or moving of existing furniture',
      ],
    },
    paymentCancel: {
      kicker: 'Payment not completed',
      titulo: 'You cancelled the payment',
      texto: 'No charge was made. You can try again whenever you like.',
      volverCatalogo: 'Back to catalog',
    },
    admin: {
      cargando: 'Loading…',
      cerrarSesion: 'Log out',
      panelPrivado: 'Private panel',
      administracion: 'Administration',
      sinPermisos: 'You have no permissions assigned. Contact the administrator.',
      tabProductos: 'Products',
      tabCotizaciones: 'Quotes',
      tabBanner: 'Banner',
      tabCatalogo: 'Catalog',
      tabInventario: 'Inventory',
      tabVentas: 'Sales',
      tabEnvio: 'Shipping',
      tabUsuarios: 'Users',
    },
    loginForm: {
      titulo: 'Restricted access',
      subtitulo: 'Admin panel',
      correo: 'Email',
      contrasena: 'Password',
      errorCredenciales: 'Incorrect email or password.',
      entrando: 'Signing in…',
      entrar: 'Sign in',
    },
    productManager: {
      confirmarEliminar: 'Delete this piece from the catalog?',
      editarPieza: 'Edit piece',
      nuevaPieza: 'New piece',
      nombre: 'Name',
      nombreEn: 'Name (English, optional)',
      categoria: 'Category',
      subcategoria: 'Subcategory',
      sinSubcategoria: 'No subcategory',
      material: 'Material',
      materialEn: 'Material (English, optional)',
      anchoCm: 'Width (in)',
      altoCm: 'Height (in)',
      fondoCm: 'Depth (in)',
      precioDesde: 'Price from (USD)',
      descripcion: 'Description',
      descripcionEn: 'Description (English, optional)',
      imagen: 'Image',
      imagenActualAlt: 'Current',
      galeria: 'Photo gallery (additional)',
      colores: 'Available colors',
      agregarColor: 'Add color',
      colorNombrePlaceholder: 'E.g. Beige',
      colorNombreEnPlaceholder: 'English name (optional)',
      guardadoExito: 'Saved successfully ✓',
      guardando: 'Saving…',
      guardarCambios: 'Save changes',
      agregarPieza: 'Add piece',
      cancelar: 'Cancel',
      catalogoActual: 'Current catalog',
      cargando: 'Loading…',
      vacio: 'No pieces yet.',
      todasSubcategorias: 'All subcategories',
      sinResultadosFiltro: 'No pieces match this filter.',
      editar: 'Edit',
      eliminar: 'Delete',
    },
    quotesList: {
      cargando: 'Loading…',
      vacio: 'No quote requests yet.',
      confirmarEliminar: 'Delete this quote?',
      montoInvalido: 'Enter a valid amount.',
      eliminar: 'Delete',
      telefono: 'Phone:',
      correo: 'Email:',
      tipoMueble: 'Furniture type:',
      presupuesto: 'Budget:',
      montoTotalPlaceholder: 'Total price of the piece (USD, optional)',
      anticipoRecibido: 'Deposit received ✓ —',
      montoPlaceholder: 'Deposit amount (USD)',
      restoRecibido: 'Remaining balance received ✓ —',
      montoRestoPlaceholder: 'Remaining balance amount (USD)',
      generarLinkResto: 'Generate payment link for the balance',
      generando: 'Generating…',
      generarLink: 'Generate payment link',
      copiar: 'Copy',
    },
    userManager: {
      nuevoUsuario: 'New user',
      correo: 'Email',
      contrasena: 'Password',
      rol: 'Role',
      usuarioEstandar: 'Standard user',
      adminRol: 'Admin',
      accesoSecciones: 'Section access',
      productos: 'Products',
      cotizaciones: 'Quotes',
      banner: 'Banner',
      catalogo: 'Catalog',
      inventario: 'Inventory',
      ventas: 'Sales',
      envio: 'Shipping',
      creando: 'Creating…',
      crearUsuario: 'Create user',
      usuariosExistentes: 'Existing users',
      cargando: 'Loading…',
      vacio: 'No users yet.',
      guardando: 'Saving…',
      guardar: 'Save',
      eliminar: 'Delete',
      confirmarEliminar: 'Delete this user? They will lose access immediately.',
      mostrarPassword: 'Show password',
      ocultarPassword: 'Hide password',
    },
    inventoryManager: {
      titulo: 'Inventory',
      cargando: 'Loading…',
      vacio: 'No pieces in the catalog yet.',
      sinExistencias: 'Out of stock',
      bajoStock: 'Low stock',
      guardando: 'Saving…',
      guardado: 'Saved ✓',
      guardar: 'Save',
      guardarTodo: 'Save all',
      errorGuardarTodo: 'Some changes could not be saved.',
    },
    salesManager: {
      titulo: 'Sales',
      cargando: 'Loading…',
      vacio: 'No sales in this period.',
      hoy: 'Today',
      semana: 'Weekly',
      mes: 'Month',
      anio: 'Year',
      fecha: 'Date',
      tipo: 'Type',
      descripcion: 'Description',
      cliente: 'Customer',
      monto: 'Amount',
      total: 'Total',
      tipoProducto: 'Direct purchase',
      tipoCarrito: 'Cart',
      tipoAnticipo: 'Deposit',
      generarReporte: 'Generate PDF reports',
      generando: 'Generating…',
      reporteTitulo: 'Sales report',
      reportePiezasTitulo: 'Units sold report',
      periodo: 'Period',
      generadoEl: 'Generated on',
      vistaDinero: 'Money',
      vistaPiezas: 'Units sold',
      categoria: 'Category',
      piezasVendidas: 'Units sold',
      expandir: 'Show subcategories',
      colapsar: 'Hide subcategories',
    },
    shippingManager: {
      titulo: 'Shipping',
      descripcion: 'Set the freight delivery amount shown before checkout.',
      cargando: 'Loading…',
      montoFlete: 'Freight amount (USD)',
      guardando: 'Saving…',
      guardar: 'Save',
      guardadoExito: 'Saved successfully ✓',
    },
    bannerManager: {
      cargando: 'Loading…',
      tituloInicio: 'Home Banner (carousel)',
      descripcionInicio: 'These images rotate in the Home page banner. Add or remove as many as you like.',
      tituloCatalogo: 'Catalog Banner (fixed image)',
      descripcionCatalogo: 'A single fixed image shown above the listing on /catalogo.',
      agregarImagenes: 'Add images',
      cambiarImagen: 'Change image',
      subiendo: 'Uploading…',
      confirmarQuitar: 'Remove this image from the banner?',
      errorSubir: 'Could not upload the image',
    },
    catalogoManager: {
      cargando: 'Loading…',
      descripcion:
        'The cover is the photo shown on the card when entering /catalogo. The gallery is several photos shown when entering that category.',
      portada: 'Cover (1 photo)',
      sinPortada: 'No cover yet',
      subirPortada: 'Upload cover',
      galeria: 'Catalog gallery (several photos)',
      sinGaleria: 'No photos yet',
      agregarGaleria: 'Add photos',
      subiendo: 'Uploading…',
      confirmarQuitar: 'Remove this image?',
      errorSubir: 'Could not upload the image',
    },
    idioma: { cambiarA: 'ES' },
  },
}

export function traducirCategoria(categoria, lang) {
  return translations[lang]?.categorias?.[categoria] ?? categoria
}

export function traducirSubcategoria(subcategoria, lang, categoria) {
  return translations[lang]?.subcategorias?.[categoria]?.[subcategoria] ?? subcategoria
}
